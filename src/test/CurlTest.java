...
public class CurlTest {
  ...
  /**
   * Stores the root of file system
   */
  FileSystemI root;
  ...
  /**
   * the url that user passes
   */
  
  String url;
  
  /**
   * the content inside the url
   */
  String content;
  
  /**
   * This func sets up the root, url and the content of the url
   */
  @Before
  public void setUp() {  
    url = "http://www.cs.cmu.edu/~spok/grimmtmp/073.txt";
    ...
    content = "There was once a king who had an illness, and no one believed that he\n"
  ...
  /**
   * This func checks if the curl command creates duplicate files or not
   */
  @Test
  public void testCheckDuplicates() {
    ...
    boolean b = curl.checkForDuplicates("File", root);
    assertEquals(true, b);
  }
  
  /**
   * This func tests if the curl command actually creates a file with correct name
   */
  @Test
  public void testCreateFile() {
    
    curl.createFile(url, content, root);
    
    ArrayList<File> subFiles = root.getAllSubFiles();
    
    if(subFiles.get(0).getName().equals("073txt")
        && subFiles.get(0).getcontent().equals(content)) {
      assertTrue(true);
    }
    else {
      fail();
  ...
  /**
   * this func check if the file that curl command creates contains the correct content
   */
  @Test
  public void testReadUrl() {
    curl.readUrl(url, root);
